...
**One programmable canvas across every Stream Deck key.**
...
![KeyGrid Canvas running the signal scene across a Stream Deck XL](docs/assets/xl-signal-deck.svg)
...
The easiest route is the packaged `com.keygrid.canvas.streamDeckPlugin` file
from a GitHub release. Double-click it, then add **Canvas Tile** to every key
on one Stream Deck page.
...
- A Stream Deck with keypad keys.
